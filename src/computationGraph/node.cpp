...
std::vector<AbstractNode *> AbstractNode::getPreviousNodes() const { return prevs; }
...
NodeInterface NodeInterface::MatrixMult(const NodeInterface& left, const NodeInterface& right) {
    checkSameGraph(left, right);
    AbstractOperation multiplication("matrix_multiplication");
    std::string name = createName();
    return left.graph.AddOperationNode(multiplication, {left.node, right.node}, name, false);
}
