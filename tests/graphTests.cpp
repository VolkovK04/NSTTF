...
TEST(GraphTests, MatrixMultNode) {
  ...
  NodeInterface nodeInterface1 = g.AddInputNode("test1");
  NodeInterface nodeInterface2 = g.AddInputNode("test2");
  NodeInterface multNode =
      NodeInterface::MatrixMult(nodeInterface1, nodeInterface2);
  const OperationNode *node =
      static_cast<const OperationNode *>(&multNode.getNode());
...
TEST(GraphTests, MatrixTranspose) {
  ...
  const OperationNode *node =
      static_cast<const OperationNode *>(&trNode.getNode());
